...
def save_Mask(mask, saveMaskRoot, saveMaskPatient, z_axis, h, w, clsNames):
    ...
    for cls in classes:
        ...
        for con in contours:
            ...
            for saveP in saveMaskRoot:  # [CT path, PET path]
                cv2.imwrite(f'{saveP}/{saveMaskPatient}/{z_axis}_{int(cls)}_{clsNames[int(cls)]}_{eq_cls_cnt}.png', mask_zeros)
...
def main(config_file):
    ...
    #* (2) images/labels 얻기
    filesSeg = glob(cfg["segP"])                #* # of studies = 1014
    # pet/ct 다 불러오기
    filesImg_CT  = [seg.replace('SEG.nii.gz', 'CTres.nii.gz') for seg in filesSeg]
    filesImg_PET = [seg.replace('SEG.nii.gz', 'PET.nii.gz') for seg in filesSeg]
    
    #* (3) save
    for idx in tqdm(range(len(filesImg_PET))):
        segName = filesSeg[idx]
        imgName_CT, imgName_PET = filesImg_CT[idx], filesImg_PET[idx]
        studyName = segName.split('/')[-2]
        
        os.makedirs(f'{cfg["save_PET_segP"]}/{studyName}', exist_ok=True)
        os.makedirs(f'{cfg["save_PET_imgP"]}/{studyName}', exist_ok=True)
        os.makedirs(f'{cfg["save_CT_segP"]}/{studyName}', exist_ok=True)
        os.makedirs(f'{cfg["save_CT_imgP"]}/{studyName}', exist_ok=True)        
        #* 파일 읽기
        _, segdata = rutils.read_by_format(segName, cfg["format_seg"])
        _, imgdata_CT = rutils.read_by_format(imgName_CT, cfg["format_img"])
        _, imgdata_PET = rutils.read_by_format(imgName_PET, cfg["format_img"])

        if imgdata_CT.shape != segdata.shape or imgdata_PET.shape != segdata.shape:
        ...
        for _z in range(z):
            if segdata.shape[0] == segdata.shape[1]:
                mask, img_ct, img_pet = segdata[:,:,_z], imgdata_CT[:,:,_z], imgdata_PET[:,:,_z]
            elif segdata.shape[0] > segdata.shape[-1]:
                mask, img_ct, img_pet = segdata[:,:,_z], imgdata_CT[:,:,_z], imgdata_PET[:,:,_z]
            else:
                mask, img_ct, img_pet = segdata[_z], imgdata_CT[_z], imgdata_PET[_z]
            ...
            #* mask 처리
            is_save_img = save_Mask(mask, [cfg["save_CT_segP"], cfg["save_PET_segP"]], studyName, _z, h, w, cfg["CLASSES_NAME"])
            #* image 처리
            if is_save_img:
                for img, saveP in zip([img_ct, img_pet], [cfg["save_CT_imgP"], cfg["save_PET_imgP"]]):
                    img = cutils.normalizePlanes(img) * 255
                    cv2.imwrite(f'{saveP}/{studyName}/{_z}.png', img)
    
if __name__ == "__main__":

    main(
        config_file = '/home/nute11a/workspace/SAMM/0_Data_preprocessing/configs/AutoPET.json'
    )
