...
def getImageFiles(segfiles, ds):
    ...
    if ds == 'ToothFairy':
        return [seg.replace('gt_sparse', 'data') for seg in segfiles]
...
def getPatientName(imgName, cfg):
    if cfg['dataset_name'] == 'VALDO-Task2' or cfg['dataset_name'] == 'ToothFairy':
...
def main(config_file):
    ...
    for idx in tqdm(range(len(filesImg))):
        ...
        if cfg["format"] in ['.nii.gz', '.nrrd', '.nii']:
            h, w, z = segdata.shape
        elif cfg["format"] in ['.mha', '.mhd', '.npy']:
            z, h, w = segdata.shape
...
if __name__ == "__main__":
    ...
    main(
        config_file = './configs/InnerEarSeg.json'
    )
