    ...
    for cls in classes:
        ...
        for con in contours:
            ...
            if type(clsNames) == 'dict':
                cv2.imwrite(f'{saveMaskRoot}/{saveMaskPatient}_{int(cls)}_{clsNames[int(cls)]}_{eq_cls_cnt}.png', mask_zeros)
            else:
                cv2.imwrite(f'{saveMaskRoot}/{saveMaskPatient}_{1}_{clsNames[1]}_{eq_cls_cnt}.png', mask_zeros)
...
if __name__ == "__main__":
    main(
        config_file = './configs/2D/mitoEM.json'
    )