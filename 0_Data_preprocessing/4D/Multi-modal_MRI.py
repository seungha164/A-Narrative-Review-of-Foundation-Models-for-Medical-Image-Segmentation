...
def saveMask(mask, saveMaskRoot, saveMaskPatient, z_axis, h, w, clsNames, views):
    is_save_img = False
    #! Multi-class 처리
    # 1. class 추출
    ...
    for cls in classes:
        ...
        for con in contours:
            ...
            for v in views:
                cv2.imwrite(f'{saveMaskRoot}/{saveMaskPatient}/{z_axis}_{v}_{int(cls)}_{clsNames[int(cls)]}_{eq_cls_cnt}.png', mask_zeros)   
...
def main(config_file):
    ...
    for idx in tqdm(range(len(filesImg))):
        ...
        # check
        if imgdata[:,:,:,0].shape != segdata.shape:
        ...
        for _z in range(z):
            ...
            #* mask 처리
            is_save_img = saveMask(mask, cfg["save_segP"], patientName, _z, h, w, cfg["CLASSES_NAME"], cfg["view"])
            #* image 처리
            if is_save_img:
                for idx, v in enumerate(cfg["view"]):
                    img = normalizePlanes(img) * 255
                    cv2.imwrite(f'{cfg["save_imgP"]}/{patientName}/{_z}_{v}.png', img[:, :, idx])

if __name__ == "__main__":
    main(
        config_file = '../configs/MSD-Brain.json'
    )